...
def main():
    ...
    try:
        with open(os.path.join(args.save_dir, args.task, f"{args.subset}.jsonl"), 'r') as json_file:
            json_list = list(json_file)
            if len(json_list) == num_samples:
                print(f"File with samples already exists.")
                return
    except:
        print(f"File with samples does not exist.")
