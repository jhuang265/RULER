...
parser.add_argument("--fixed_depth", type=float, default=None, help='[Optional] A fixed (relative) position at which to place the needle. Needs to be between 0 and 1.')
...
def generate_input_output(num_haystack):
    ...
    if args.type_haystack == 'essay':
        ...
        if args.fixed_depth is not None:
            ...
            insertion_positions = [0] + \
                                sorted([int(len(document_sents) * args.fixed_depth) for _ in range(len(needles))]) + \
                                [len(document_sents)] 
        ...
        if args.fixed_depth is not None:
            ...
            indexes = sorted([int(num_haystack * args.fixed_depth) for _ in range(len(needles))], reverse=True)
